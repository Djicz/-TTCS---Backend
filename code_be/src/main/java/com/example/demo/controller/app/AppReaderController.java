...
public class AppReaderController {
    ...
    public ResponseEntity<?> readChapter(@PathVariable Long chapterId,
        ...
        try {
            ...
            response.put("nextChapterId", chapterService.getNextChapterId(chapter.getStory().getId(), chapter.getChapterNumber()));
            response.put("prevChapterId", chapterService.getPrevChapterId(chapter.getStory().getId(), chapter.getChapterNumber()));
